...
var ht = 16; //height
var wh = Math.floor($(window).width()/sz) - 4 ; //width
...
{ 
  ...
  function isBody(col,row){
    for(body of sn.bod){
      if(body.x === col && body.y === row){
        return true;
      }
    }
    return false;
  ...
  if(fstate === "NO")                 
  {
    ...
    for(row=0;row<ht;row++){
      for(col=0;col<wh;col++){
        if(!isBody(col,row)){
          valid.push([col,row]);
        }
      }
    }
    random_ind = Math.round(random(valid.length-1));
    food.x = valid[random_ind][0];
    food.y = valid[random_ind][1];
    fstate = "YES";
  }
...
function draw() {
  ...
  else if(state === "OFF")
  {
    ...
    textAlign(CENTER,CENTER);
    fill(255);
    text('GAME OVER\n\n Your score is: ' + sn.len + '\n Hit "Space" to restart!',wh*sz/2,ht*sz/2);
  }
  ...
  else if(state === "IDLE")
  {
    ...
    textAlign(CENTER,CENTER);
    fill(255);
    text('Oops you clicked out of the window\n\nClick back in to continue!',wh*sz/2,ht*sz/2);
